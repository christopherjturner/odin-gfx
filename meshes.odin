...
init_static_meshes :: proc() -> Static_Mesh_Renderer {
	...
	static_mesh_renderer.bind.views[shaders.VIEW_mesh_tex] = sg.make_view({texture = {image = texture}})
...
draw_static_meshes :: proc(static_mesh_renderer: ^Static_Mesh_Renderer, camera: ^Camera, dt: f32) {
	...
	sg.apply_uniforms(shaders.UB_static_mesh_vs_params, {
        ptr = &vs_params,
        size = size_of(vs_params)
    })
	sg.draw(0, static_mesh_renderer.models[0].mesh.index_count, 1)
    add_aabb(state.aabb, static_mesh_renderer.models[0].mesh.aabb, static_mesh_renderer.models[0].transform)
}
